...
def calmetric_new(pred_recon, gt_recon):
    if gt_recon.ndim == 4:
        psnr_array = np.zeros((gt_recon.shape[-2], gt_recon.shape[-1]))
        ssim_array = np.zeros((gt_recon.shape[-2], gt_recon.shape[-1]))
        nmse_array = np.zeros((gt_recon.shape[-2], gt_recon.shape[-1]))
        ...
        ssim_array = np.zeros((1, gt_recon.shape[-1]))
        nmse_array = np.zeros((1, gt_recon.shape[-1]))
        dists_array = np.zeros((1, gt_recon.shape[-1]))
        haar_array = np.zeros((1, gt_recon.shape[-1]))
        dists = DISTS().cuda()
        for j in range(gt_recon.shape[-1]):
            pred, gt = pred_recon[:, :, j], gt_recon[:, :, j]
            psnr_array[0,j] = psnr(gt / gt.max(), pred / pred.max())
            ssim_array[0,j] = ssim(gt / gt.max(), pred / pred.max())
            nmse_array[0,j] = nmse(gt / gt.max(), pred / pred.max())
            dists_array[0,j] = 1-dists(torch.tensor(gt / gt.max()).unsqueeze(0).unsqueeze(0).float().cuda(), torch.tensor(pred / pred.max()).unsqueeze(0).unsqueeze(0).float().cuda()).cpu().numpy()
            haar_array[0,j] = haarpsi(torch.tensor(gt / gt.max()).unsqueeze(0).unsqueeze(0).float(), torch.tensor(pred / pred.max()).unsqueeze(0).unsqueeze(0).float()).numpy()

    return psnr_array, ssim_array, nmse_array, dists_array, haar_array
